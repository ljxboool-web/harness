...
function startNarrate() {
  ...
  es.addEventListener('attempt', (e) => {
    ...
    chip.textContent = `第 ${d.attempt} 次 · median ${d.median_score}/5 · [${indiv}]`;
  ...
  es.addEventListener('done', (e) => {
    ...
    chip.textContent = `完成 · median ${d.judge.median_score}/5 · [${indiv}] · ${d.trace.length} 轮`;
...
function renderNarrativeBody(done) {
  ...
  if (!parts.strong && !parts.weak && !parts.advice) {
    body.innerHTML = `<pre class="narrative-raw">${escapeHTML(text)}</pre>`;
    return;
  }
  body.innerHTML = `
