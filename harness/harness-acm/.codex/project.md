...
- Default LLM path is Alibaba Cloud DashScope Responses API via Qwen models.
- Without `DASHSCOPE_API_KEY`, narrative generation must fall back to the template path.
...
- After prompt or judge changes: run CLI once with and once without `DASHSCOPE_API_KEY`
