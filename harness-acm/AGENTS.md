...
3. 基于客观数据生成评估报告，并优先使用 OpenAI Responses API 的 Codex 模型
...
└──────┬───────┘  （纯统计，不评分）
       ▼
┌──────────────┐
│  analyzer.py │   8 维技能 + 5 维特征打分 + OpenAI/Codex 评语 → AbilityReport
└──────┬───────┘
       ▼
┌──────────────┐
...
- LLM 默认使用 OpenAI Responses API；缺少 `OPENAI_API_KEY` 时退化到模板评语，不得伪造已调用模型。
...
- 改 prompt 或 judge 流程后，至少跑一次 `python src/cli.py tourist --no-ai` 和一次带 `OPENAI_API_KEY` 的完整链路。
...
- `OPENAI_JUDGE_MODEL`: judge 使用的模型，默认跟随 `OPENAI_MODEL`
- `OPENAI_REASONING_EFFORT`: narrative 推理强度，默认 `medium`
- `OPENAI_JUDGE_REASONING_EFFORT`: judge 推理强度，默认 `low`
