"""Judge 重写循环测试 — 用 monkeypatch 模拟 ensemble 响应，验证重试逻辑."""
...
def _ensemble(score: int, reason: str = "ok") -> JudgeEnsembleResult:
    """Build a JudgeEnsembleResult where all three judges return the same score."""
    individuals = [
        JudgeResult(score=score, reason=reason, judge_name=name)
        for name in ("strict", "lenient", "data")
    ]
    return JudgeEnsembleResult(
        median_score=score, individual=individuals,
        combined_reason=f"[strict/{score}]{reason}",
    )
...
def test_no_key_single_attempt(monkeypatch):
    ...
    assert len(trace) == 1
    assert judge.median_score == 3
    assert "API key" in judge.combined_reason
    assert narrative  # 非空
...
def test_exhaust_retries(monkeypatch):
    ...
    assert judge.median_score == 2  # 仍然失败
...
def test_judge_receives_populated_narrative(monkeypatch):
    ...
    def fake_judge(r):
        ...
        return _ensemble(5, "ok")
    monkeypatch.setattr(analyzer, "judge_report_ensemble", fake_judge)
...
def test_on_attempt_called(monkeypatch):
    ...
    analyzer.generate_narrative_with_judge(
        ...
        on_attempt=lambda n, j: calls.append((n, j.median_score)),
    )
    assert calls == [(1, 3), (2, 5)]


# ---- trace 里包含 individual_scores ----

def test_trace_has_individual_scores(monkeypatch):
    monkeypatch.setenv("DASHSCOPE_API_KEY", "fake-key-for-test")
    monkeypatch.setattr(analyzer, "generate_narrative",
                        lambda r, feedback=None: "x")
    monkeypatch.setattr(analyzer, "judge_report_ensemble",
    ...
    assert "individual_scores" in trace[0]
    assert trace[0]["individual_scores"] == [5, 5, 5]
